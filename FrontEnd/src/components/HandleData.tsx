        ...
        <div className="mt-20 flex flex-col gap-6 ">
            {links.length ? 
                links.map(link =>(
                    ...
                    href={link.url}
                    target="_blank"
                    rel="norefrrer noopener"

                    >
                        <img src={`/social/icon_${link.name}.svg`} alt="Imagen SocialNetwork" className="w-12" />
                        <p className="text-black capitalize text-lg">Visita mi: <span className="font-bold">{link.name}</span></p>
                    </a>
                ))
            : <p className="text-lg text-center font-bold">No hay enlaces en este perfil</p>}
        </div>
    </div>
  )
}
